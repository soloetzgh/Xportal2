...
//                );
//            }])
//        .run(function ($route, $rootScope, profileService, $location, menuList, menus, $http, $httpParamSerializerJQLike) {
...
//                }, 250);
//            });
//            $rootScope.checkOpenTo = function (accessLevel, profile, openTo) {
//                var allowed = false;
//
//                if (profile !== undefined && openTo !== undefined) {
////                    console.log("============================");
...
//
//
////                        if (type === '2500000000000049') {
////
////                            console.log('stats: ' + stats);
////                            console.log('allowUser: ' + allowUser);
////                            console.log('accessLevel: ' + accessLevel);
////                            console.log('(profile.type_id: ' + profile.type_id);
////                            console.log('profile.type_id.indexOf(accessLevel)' + profile.type_id.indexOf(accessLevel));
////                            console.log('openTo: ' + openTo + 'type:' + type + ' = ' + profile.user_code + ' [] roles:' + roles + ' alloe: ' + allowUser);
////
////                        }
//                        if (profile.type_id.indexOf(accessLevel) > -1 && allowUser) {
//                            allowed = true;
//                        }
//                        if (profile.type_id.indexOf('[1]') > -1 || openTo.indexOf('[1]') > -1) {
//                            allowed = true;
//                        }
//                    });
//                }
...
//                    return (userProfile !== "" && userProfile !== "{}" && userProfile.message !== "Session Expired" && userProfile.isLoggedIn);
//                },
...
////                                return true;
////                            }
//                            if ($rootScope.checkOpenTo(role, userState, openTo)) {
//                                return true;
//                            } else {
...
//        .controller('authCtrl', function ($scope, userInfo, $rootScope, profileService, $http, $httpParamSerializerJQLike, $location, $compile) {
...
////                    profileService.updateUserProfile();
//                    $http.get('../XPortalProxy/api/UserProfile').then(function (success) {
////                                console.log('updating profile');
//                        if (success.data) {
...
////            $scope.initInfo();
//        })
//        .controller('fgTransactionsCtrl', function ($scope, Idle, Keepalive, userInfo, profileService, $rootScope, $parse, $http, $httpParamSerializerJQLike, $location, $timeout, $compile, Upload, $q, $route, $routeParams) {
////    $scope.profile = "";
...
////                        console.log('action: ' + $scope.action);
////                        $scope.searchUserProfile[0].status_id = (resp === 'SUCCESS' ? ($scope.action === 'activate' ? 1 : 2) : $scope.searchUserProfile[0].status_id);
//                        var respText = (resp === 'SUCCESS' ? 'New Password sent to ' + $scope.searchUserProfile[0].username : (resp === 'FAILED' ? 'Failed to reset ' + $scope.searchUserProfile[0].username + '\'s password.' : resp));
//                        var respType = (resp === 'SUCCESS' ? 'success' : 'error');
//                        var respTitle = (resp === 'SUCCESS' ? 'Password Reset Success' : 'Password Reset Failed');
//
//                        Swal.fire({title: respTitle, text: respText, type: respType, timer: 1500});
//                    }
//                });
...
//
//
//                $scope.fgRecordsTotalCount = $rootScope[data].length;
//                $scope.fgRecordsLastPage = $scope.fgRecordsTotalCount / $scope.rowsPerPage;
//                if ($scope.fgRecordsLastPage > parseInt($scope.fgRecordsTotalCount / $scope.rowsPerPage)) {
//                    $scope.fgRecordsLastPage = parseInt($scope.fgRecordsTotalCount / $scope.rowsPerPage) + 1;
//                }
...
//                    $scope.fgRecordsTotalAmount = $scope.sumAmount($rootScope[data], resp_field, resp_msg, amount_field);
//                    if (fee_field) {
//
//                        $scope.fgRecordsTotalFee = $scope.sumAmount($rootScope[data], resp_field, resp_msg, fee_field);
//                    }
//                }
//
//            };
...
//
//                $http.get('../XPortalProxy/api/CardBalance?action=' + type).then(
//                        function (response) {
//                            $scope.balance = response.data;
//                            $scope.lastBalanceDate = moment().format('DD-MM-YY H:mm');
//                        }
...
//                    $scope.end = parseInt($scope.begin) + parseInt($scope.rowsPerPage);
////                console.log('begin: ' + $scope.begin + ' : ' + $scope.end);
//                    $rootScope['filtered' + modelName] = $rootScope[modelName].slice($scope.begin, $scope.end);
//
//                    $scope.showLoader = false;
//                    $('.reload').removeClass('fa-spin text-info');
//                    $rootScope.hideLoading();
...
//
//                if (type === 'balance') {
//                    $http.get('../XPortalProxy/api/' + path + '?action=balance&merchant=' + $scope.merchant).then(function (response) {
//                        $scope.balance = response.data;
//                        $scope.lastBalanceDate = moment().format('DD-MM-YY H:mm');
//                    });
//                }
//                if (action === 'merchants' && type) {
//
//
//                    $http.get('../XPortalProxy/api/' + path + '?action=' + action + (type === 'bank' ? ('&bank=' + $scope.bank) : ('&merchant=' + $scope.merchant))).then(function (response) {
//                        $scope[modelName] = response.data;
//                    });
//                } else {
//                    $http.get('../XPortalProxy/api/' + path + '?action=' + action).then(function (response) {
//                        $scope[modelName] = response.data;
//                    });
//                }
//
...
//                        console.log('calling');
//                        return $http({
//                            url: "../XPortalProxy/api/MobileAppMgmt",
//                            method: 'POST',
//                            data: $httpParamSerializerJQLike({
//                                "clientId": client_id,
//                                "mobile_no": phone,
...
//                        var swal_html = '<div class="panel" style="background:aliceblue;font-weight:bold">\n\
//                            <div class="panel-heading panel-info text-center btn-info"> <b>Reversal Details</b> </div>\n\
//                         <div class="panel-body">\n\
//                        <div class="text-center"><b><p style="font-weight:bold">Amount: GHS ' + $scope.getAmt($scope.tmcTrxs[0].amount) + '</p><p style="font-weight:bold">Reversal Status: ' + ($scope.tmcTrxs[0].response_code === "00" ? "SUCCESSFUL" : "FAILED") + '</p></b></div>\n\
//                        </div>\n\
//                        </div>';
//                        Swal.fire({title: reference, html: swal_html});
//                    } else {
//                        $rootScope.toggleErrorNotification(response.data.message);
//                        $scope.tmcTrxs = "";
//                    }
...
//                    $rootScope.toggleErrorNotification('Connection or Sever Error. Try again');
//                    $rootScope.hideLoading();
//                });
//
//            };
...
//                $http.get('../XPortalProxy/api/MerchantsInfo?action=appIdList').then(
//                        function (response) {
...
//                $http.get('../XPortalProxy/api/WalletBalance?action=fetchBuckets').then(
//                        function (response) {
...
//                    $http.get('../XPortalProxy/api/WalletBalance?action=fetchBucketsBalance&type=' + value.alias).then(
//                            function (response) {
...
//                $http.get('../XPortalProxy/api/MerchantsInfo?action=' + ($rootScope.testEtz() ? $scope.bank : j.split(":")[1]).toLowerCase() + 'Merchants').then(
//                        function (response) {
//                            $scope.merchantList = response.data;
//                        });
//            };
...
//                    var status = $scope.filtereduserProfiles[$scope.userIndex].status_id;
...
//                html += '<option ng-repeat="option in userTypeList" title="" value="{{option.id}}">{{option.name}}</option>';
//                html += '</select>';
//                html += '</td>';
//                html += '</tr>';
...
//            };
//
////            $scope.typeIdList = [{"id": "2500000000000046", "name": "ABINATIONAL"}, {"id": "43", "name": "ACCESS"}, {"id": "59", "name": "ACCESSVIEW"}, {"id": "48", "name": "ACCOUNTS"}, {"id": "1", "name": "ADMINISTRATOR"}, {"id": "53", "name": "ADVANS"}, {"id": "41", "name": "AFB"}, {"id": "31", "name": "AGENT"}, {"id": "24", "name": "AIRTEL"}, {"id": "2500000000000044", "name": "AMARD"}, {"id": "67", "name": "AWA"}, {"id": "5", "name": "BANK ADMINISTRATOR"}, {"id": "2500000000000041", "name": "BANK COLLECTION"}, {"id": "4", "name": "BANK OFFICER"}, {"id": "52", "name": "BANK REPORT"}, {"id": "29", "name": "BARCLAYS"}, {"id": "49", "name": "BARCLAYS-CMS"}, {"id": "54", "name": "BEST-POINT"}, {"id": "70", "name": "BILL-PAYMENT"}, {"id": "26", "name": "BOA"}, {"id": "2500000000000047", "name": "BULK VERIFIER"}, {"id": "1500000000000035", "name": "BUSY"}, {"id": "79", "name": "CARD DETAIL"}, {"id": "30", "name": "CCML"}, {"id": "63", "name": "CCMLCARDREPORT"}, {"id": "51", "name": "CMS-AUTH"}, {"id": "50", "name": "CMS-INIT"}, {"id": "2000000000000048", "name": "CORPORATE PAY"}, {"id": "27", "name": "DSTV"}, {"id": "1500000000000037", "name": "ELAC"}, {"id": "2000000000000066", "name": "ELIHO"}, {"id": "2500000000000043", "name": "ELIHOREPORT"}, {"id": "21", "name": "ENERGY"}, {"id": "46", "name": "ETRXN"}, {"id": "2000000000000051", "name": "ETZ"}, {"id": "69", "name": "ETZ BUS"}, {"id": "7", "name": "ETZ STAFF"}, {"id": "8", "name": "ETZ STAFF2"}, {"id": "22", "name": "ETZ TECH"}, {"id": "44", "name": "ETZ TECH2"}, {"id": "2000000000000057", "name": "EXT REQUESTS"}, {"id": "32", "name": "FAB"}, {"id": "37", "name": "FAB2"}, {"id": "34", "name": "FCP"}, {"id": "2000000000000063", "name": "FEECOLLECTION"}, {"id": "45", "name": "FIRSTBANC"}, {"id": "73", "name": "FNB"}, {"id": "17", "name": "FUNDGATE"}, {"id": "72", "name": "GAC"}, {"id": "64", "name": "GBC"}, {"id": "61", "name": "GBCTV"}, {"id": "16", "name": "GCB"}, {"id": "23", "name": "GCB SUPPORT"}, {"id": "56", "name": "GCB-COLLECTIONS"}, {"id": "55", "name": "GCB-MERCHANT"}, {"id": "2000000000000040", "name": "GCB-MERCHANTPAY"}, {"id": "2000000000000041", "name": "GCBREPORT"}, {"id": "74", "name": "GHACEM"}, {"id": "12", "name": "GHANA-MART"}, {"id": "93", "name": "GLO"}, {"id": "78", "name": "GLOBALACCESS"}, {"id": "75", "name": "GLOBALACCESS"}, {"id": "2000000000000058", "name": "GMONEY"}, {"id": "96", "name": "GOLDEN LINK SAVINGS AND LOAN"}, {"id": "92", "name": "GPACK"}, {"id": "40", "name": "GTBANK"}, {"id": "1500000000000036", "name": "GWCL"}, {"id": "58", "name": "HUAWEI"}, {"id": "1500000000000039", "name": "ICGC"}, {"id": "94", "name": "INFOBIB"}, {"id": "2000000000000054", "name": "JUSTPAY"}, {"id": "2000000000000055", "name": "JUSTPAY_"}, {"id": "65", "name": "KNUST"}, {"id": "2000000000000043", "name": "KU LEUVEN"}, {"id": "95", "name": "KWESSE TV"}, {"id": "2000000000000049", "name": "MerchantPay"}, {"id": "47", "name": "MerchantTrnx"}, {"id": "20", "name": "MIDLAND"}, {"id": "91", "name": "MOBILE MONEY"}, {"id": "2000000000000045", "name": "MOH-MCP"}, {"id": "42", "name": "MTN"}, {"id": "99", "name": "MTN"}, {"id": "39", "name": "MTN1"}, {"id": "2000000000000050", "name": "MTNCMS"}, {"id": "2000000000000042", "name": "MTNREPORT"}, {"id": "33", "name": "NITA"}, {"id": "9", "name": "OTHERS"}, {"id": "10", "name": "OTHERS1"}, {"id": "2000000000000064", "name": "OVA BALANCE"}, {"id": "77", "name": "PAGA"}, {"id": "2500000000000042", "name": "PAYFLUID"}, {"id": "80", "name": "PING-EXPRESS"}, {"id": "19", "name": "PROFPAY"}, {"id": "3", "name": "REPORT OFFICER"}, {"id": "6", "name": "REPORT VIEWER"}, {"id": "98", "name": "SAS"}, {"id": "2000000000000047", "name": "SCB"}, {"id": "90", "name": "SEED FUND"}, {"id": "18", "name": "SIKA CASH"}, {"id": "13", "name": "SMS-GHANA"}, {"id": "28", "name": "STANCHART"}, {"id": "2", "name": "SUPPORT OFFICER"}, {"id": "57", "name": "SW-GLOBAL"}, {"id": "2000000000000060", "name": "TMC"}, {"id": "14", "name": "TRANSFER-TO"}, {"id": "2500000000000045", "name": "TREE"}, {"id": "76", "name": "UBA-PUBSEC"}, {"id": "1500000000000038", "name": "UBA-SUPPORT"}, {"id": "68", "name": "UCC-CCE"}, {"id": "66", "name": "UG-COLLECTIONS"}, {"id": "2000000000000046", "name": "UGFP"}, {"id": "15", "name": "UMOBILE"}, {"id": "11", "name": "UNION"}, {"id": "97", "name": "UNIVERSAL MERCHANT BANK"}, {"id": "36", "name": "UT-BANK"}, {"id": "2000000000000056", "name": "VASGATE"}, {"id": "2000000000000062", "name": "VASGATETOPUP"}, {"id": "2000000000000053", "name": "VASMERCHANT"}, {"id": "2000000000000044", "name": "VODAFONE"}, {"id": "35", "name": "VODAFONE"}, {"id": "71", "name": "WEBCONNECT"}, {"id": "38", "name": "WEBUSER"}, {"id": "25", "name": "WWBG"}];
...
////                html += '<option value="" selected disabled>Select User Type</option>';
//                html += '<option ng-repeat="option in userTypeList" title="yesu" value="{{option.id}}">{{option.name}}</option>';
//                html += '</select>';
...
//                    }
//                    html += '<option ng-repeat="option in datafield' + $rootScope.currentIndex + '0_" value="{{option.alias}}">{{option.name}}</option>';
//                    html += '</select>';
//                    html += '</div>';
//                    html += '</td>';
//                }
