...
(function () {
    ...
    $rootScope.fetchMenuList = function () {
        ...
        return $http.get('api/PortalSettings?action=menuList').then(function (resp) {
    ...
    $rootScope.fetchPortalSettings = function () {
        ...
        return $http.get('api/PortalSettings?action=config').then(function (resp) {
            //                    console.log('dat: ' + resp.data.messag);
            //                    console.log('dat2: ' + JSON.stringify(resp.data));
            var respmesg = resp.data.message === undefined || resp.data.message !== 'Session Expired' ? resp.data : "";
            //                    console.log('fre: ' + respmesg);
...
app.config(['$httpProvider', '$routeProvider', 'IdleProvider', 'KeepaliveProvider', function ($httpProvider, $routeProvider, IdleProvider, KeepaliveProvider) {
        ...
        $ocLazyLoadProvider.config({
            ...
            modules: [{
                    name: 'lazy_assets',
                    files: ['assets/css/rwd-table.min.css',
                        //                                'assets/css/all.min.css',
                        'assets/css/bootstrap-toggle.min.css',
                        //                                'assets/css/uikit.almost-flat.min.css',
                        'assets/css/jquery.datetimepicker.min.css', 'assets/css/jquery.contextMenu.min.css', 'assets/css/daterangepicker.min.css', 'assets/js/rwd-table.js', 'assets/js/app.js', 'assets/js/chart.min.js',
                        //                                'assets/js/dynamicgraph.js',
                        'assets/js/xlsx.full.min.js', 'assets/js/ng-file-upload-shim.js', 'assets/js/ng-file-upload.js', 'assets/js/jquery.datetimepicker.full.min.js',
                        ...
                        'assets/js/jquery.stickyheader.js', 'assets/js/jquery.ba-throttle-debounce.min.js', 'assets/js/money.min.js', 'assets/js/angular-long-press.min.js', 'assets/js/jquery.contextMenu.min.js', 'assets/js/jquery.ui.position.js', 'assets/js/bootstrap-toggle.min.js', 'assets/js/filesaver.min.js', 'assets/js/jquery.dragon.js'
...
//                        console.log("cur:",cur);
                        return cur.menu === type && cur.openTo && (i.indexOf(cur.accessLevel) > -1 || (i.indexOf('[1]') > -1 || i.indexOf('[0]') > -1) && cur.openTo.length === 0) ? ++acc : acc;
                    }, 0);
                    return count;
                };
                $rootScope.defaultState = false;
...
//                        console.log('menuItem: ' + JSON.stringify(menuItem));
                        ...
                        if (profile !== undefined && openTo !== undefined && open !== undefined) {
                            ...
                            openTo.split(",").forEach(function (stats) {
                                ...
                                if (!allowUser && profile.user_code.indexOf('[' + stats + ']') > -1) {

//                                    type = stats.split("|")[0];

                                    roles = profile.user_code.split('[' + stats + ']|').pop().split(',')[0].split("~");
                                    console.log('allowUser');
                                    console.log('type: ', type);
                                    console.log('roles: ', roles);
                                    console.log('accessLevel: ', accessLevel);
                                    console.log('profile.type_id: ', profile.type_id);

                                    allowUser = roles.length > 0;
                                }

//                                console.log('profile: ', profile.type_id);
//                                                                console.log('USECODE: ', profile.user_code);
...
//                                console.log('df');
                ...
                $rootScope.testMerchantPayAll = function (type) {
                    var j = $rootScope.profile.user_code;
                    //                console.log('rec: ' + j);
                    //                console.log('rec2: ' + profileService.getUserProfile());
                    if (type) {
                        return $rootScope.testEtz() || j.indexOf('ETZ:ALL') > -1 || j.indexOf(type + ':ALL') > -1;
                    } else {
                        return  (j.indexOf('ETZ:ALL') > -1 || j.indexOf('GCB:ALL') > -1 || j.indexOf('SCB:ALL') > -1);

                    }
                    return false;
                };
                $rootScope.testMerchantPaySettlement = function (type) {
                    var j = $rootScope.profile.user_code;
                    //                console.log('rec: ' + j);
                    //                console.log('rec2: ' + profileService.getUserProfile());
...
//
//                    }
                    return $rootScope.testEtz() || j.indexOf('GCB:') > -1;
                };
            }])
        ...
        .factory('profileService', ["userInfo", "$rootScope", "$http", "$location", "$q", function (userInfo, $rootScope, $http, $location, $q) {
                var userProfile = userInfo;
                var addUserProfile = function addUserProfile(profile, type) {
                ...
                var getFirstLogon = function getFirstLogon() {
                ...
                return {
                    ...
                    reloadSessionProfile: function reloadSessionProfile() {
                        var profileExists = typeof userProfileData === "string" ? userProfileData.length > 0 : Object.keys(userProfileData).length > 0;
                    ...
                    setUserProfile: function setUserProfile(profile, type) {
                    ...
                    isFirstLogon: function isFirstLogon() {
                        //                    console.log('userProfile: ');
                        return userProfile.first_logon === "0" || userProfile.temp_password === "true";
                    },
                    authenticate: function authenticate(role, openTo, open) {
        ...
        .factory('responseInterceptor', ["$q", "$rootScope", function ($q, $rootScope) {
                return {
                    response: function response(_response) {
                        ...
                        var header = _response.headers()['content-type'];
                        if (header && header.indexOf('application/json') > -1) {
                            if ((typeof _response === "undefined" ? "undefined" : _typeof(_response)) === 'object') {
                                //                            console.log('object response');
                                //                            console.log('r -> ' + response.data.message);
                                if (_response.data.message === 'Session Expired') {
                                    $rootScope.toggleErrorNotification(_response.data.message);
                                    $rootScope.logout();
                                    return $q.reject(_response);
                                } else if (_response.data.code === '01') {

                                    $rootScope.toggleErrorNotification(_response.data.message);
                                }
                            }
                        } else if (header && header.indexOf('application/pdf') > -1) {
                            var filename = _response.headers('Content-Disposition').split("filename=")[1];
                            // console.log('filename: ' + filename);
                            saveAs(new Blob([_response.data], {type: "application/pdf"}), filename);
                        } else if (header && header.indexOf('application/octet-stream') > -1) {
                            var filename = _response.headers('Content-Disposition').split("filename=")[1];
                            //                        console.log('filename: ' + filename.replace(/\"/g, ""));
                            saveAs(new Blob([_response.data], {type: "application/octet-stream"}), filename.replace(/\"/g, ""));
                        }
                        //                      
                        return _response;
                    },
        ...
        .factory('csrfInterceptor', ["$rootScope", function ($rootScope) {
                // This interceptor is equivalent to the behavior induced by $.ajaxSetup({cache:false});
                return {
                    request: function request(config) {
                        //                    if (config.method === 'GET' || config.method === "POST") {
                        ...
                        if (config.method === "POST") {
                            // Add a query parameter named '_' to the URL, with a value equal to the current timestamp
                            console.log('sinding this: ', $rootScope.csrf);
                    ...
                    response: function response(config) {
                        ...
                        if (config.headers()['xsrf-token']) {
                            ...
                            console.log('got this: ', $rootScope.csrf);
                            if ($('.csrf').length > 0) {
        ...
        .controller('supportCtrl', ['$scope', 'userInfo', '$rootScope', 'profileService', '$http', '$httpParamSerializerJQLike', function ($scope, userInfo, $rootScope, profileService, $http, $httpParamSerializerJQLike) {
            ...
            }]).controller('noCtrl', function () {}).controller('authCtrl', ['$scope', 'userInfo', '$rootScope', 'profileService', '$http', '$httpParamSerializerJQLike', '$location', '$compile', '$routeParams', function ($scope, userInfo, $rootScope, profileService, $http, $httpParamSerializerJQLike, $location, $compile, $routeParams) {
...
//                        $(this).attr('placeholder', ''); //sets it back to the initial value
                        ...
                        var documentDefinition = {
                            content: [{
                                    ...
                                    table: {
                                        headerRows: 1,
                                        widths: ['*', '*', '*', '*'],
                                        body: [[{text: 'Header 1', style: 'tableHeader'}, {text: 'Header 2', style: 'tableHeader'}, {text: 'Header 3', style: 'tableHeader'}], [{text: 'Hello'}, {text: 'I'}, {text: 'am'}], [{text: 'a'}, {text: 'table'}, {text: '.'}]]
                                    }
                                }],
                            styles: {mainheader: {
                                    fontSize: 20,
                    ...
                    function KeyPress(e) {
                        ...
                        if (evtobj.keyCode === 90 && evtobj.ctrlKey && evtobj.shiftKey) {
                            //                alert("hello");
                            $http.get('https://api.freebinchecker.com/bin/464898').then(function (response) {
                                console.log(response.data);
                            });
                        }
                    }
                ...
                $scope.angularVersion = window.location.hash.length > 1 ? window.location.hash.indexOf('/') === 1 ? window.location.hash.substring(2) : window.location.hash.substring(1) : '1.2.24';
                $scope.invalidFiles = [];
                ...
                $scope.UploadOvaData = function (file, type, urlType) {
                    ...
                    $http.get($scope.ovaUploadProxy).then(function (response) {
                ...
                $scope.UploadKycData = function (file, type, urlType) {
                    ...
                    $scope.showProgressBarError = false;
                    formdata.append("file", file);
                    $http.get($scope.kycUploadProxy).then(function (response) {
...
//                                                                            console.log(jsonObj);
                ...
                $scope.showUploadData = function (type, delay) {
                    switch (type) {
                        case "reversal":
                            Swal.fire({
                                focusConfirm: false,
                                ...
                                preConfirm: function preConfirm(file) {
                                    ...
                                    if (!file) {
                                        ...
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'reference').then(function (res) {
                            ...
                            Swal.fire({
                                ...
                                preConfirm: function preConfirm(file) {
                                    ...
                                    if (!file) {
                                        ...
                                        var ext = filename.substring(filename.lastIndexOf('.') + 1);
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'reference,clientid').then(function (res) {
                                                return $http({
                                                    url: "api/Mobilemoney",
                                                    method: 'POST',
                                                    data: $httpParamSerializerJQLike({
                            ...
                            Swal.fire({
                                ...
                                preConfirm: function preConfirm(file) {
                                    ...
                                    //                                } 
                                        ...
                                        var ext = filename.substring(filename.lastIndexOf('.') + 1);
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'reference,account,amount,transaction_type,destination_type,extra_info').then(function (res) {
                                                //                                                    return $http({
                                                //                                                        url: "api/Reprocess",
                                                //                                                        method: 'POST',
                                                //                                                        data: $httpParamSerializerJQLike({
                            ...
                            Swal.fire({
                                ...
                                if (!result.dismiss) {
                                    ...
                                    if (result.code === '00') {
                                        ...
                                        var modelName = "VasReprocessTrxs";
                                        var respField = "response_code";
                                        var respMsg = "00";
                                        var amountColumn = "amount";
                                        $scope.pageNumber = 1;
                        ...
                        case "mobilemoneyreprocess":
                            ...
                            Swal.fire({
                                focusConfirm: false,
                                title: 'Mobile Money Reprocessing',
//                    input: 'select',
//                    inputOptions: options,
//                    html: `<input class="swal2-file" id="uploadFile" placeholder="Please select valid Excel File" accept=".xlsx" type="file"/><br /> `,
                                input: 'file',
...
//                            $rootScope.DisplayThird = false;
                        ...
                        case "emiprocess":
                            ...
                            for (var i = 0; i < $rootScope.emiAccountTypesList.length; i++) {
                                // POPULATE SELECT ELEMENT WITH JSON.
                                //                        console.log($rootScope.banksList[i]['issuer_code']);

                                html += '<option value="' + $rootScope.emiAccountTypesList[i]['id'] + '" ' + ' >' + $rootScope.emiAccountTypesList[i]['name'] + '</option>';
                            }
                            ...
                            Swal.fire({
                                ...
                                //                    html: `<input class="swal2-file" id="uploadFile" placeholder="Please select valid Excel File" accept=".xlsx" type="file"/><br /> `,
                                input: 'file',
                                html: "<span class=\"input-group-btn account_type\" style=\"margin-bottom: 20px !important\">\n\n\
                                            <select class=\"form-control\" id=\"account_type\"><option selected value=\"\" disabled>--- Select Account Type ---</option>" + html + " </select>\n\n\
                                      </span>",
                                inputAttributes: {
                                    'accept': '.xlsx',
                                    'aria-label': 'Upload Excel'
                                ...
                                showLoaderOnConfirm: true,
                                onOpen: function onOpen(ele) {
                                    $(ele).find('.account_type').insertAfter($(ele).find('.swal2-file'));
                                },
                                preConfirm: function preConfirm(file) {
                                    ...
                                    if (!file) {
                                        ...
                                        Swal.showValidationMessage('Please select account type for file');
                                    } else {
                                        var filename = $(".swal2-file").val().toLowerCase();
                                        var ext = filename.substring(filename.lastIndexOf('.') + 1);
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'ChargingPartyID,AccountNumber,TINOrGhanaCard,PhoneNumber').then(function (res) {

                                                return $http({
                                                    url: "api/Emi",
                                                    method: 'POST',
                                                    data: $httpParamSerializerJQLike({
                                                        "otherInfo": JSON.stringify(res),
                                                        "service": "uploadFile",
...
//                                        $rootScope.$apply($rootScope['filtered' + modelName] = $rootScope[modelName].slice($scope.begin, $scope.end));
                        ...
                        case "momostatuscheck":
                            //alert("Momo status check started")
                            Swal.fire({
                                focusConfirm: false,
                                title: 'Mobile Money Status Check',
                                //                    input: 'select',
                                //                    inputOptions: options,
                                //                    html: `<input class="swal2-file" id="uploadFile" placeholder="Please select valid Excel File" accept=".xlsx" type="file"/><br /> `,
                                input: 'file',
                                        ...
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'reference').then(function (res) {
                                                ...
                                                return $http({
                                                    ...
                                                    data: $httpParamSerializerJQLike({
                                                        "uniqueTransId": JSON.stringify(res),
                                                        "service": "mobileMoneyStatusCheck"
                                                    }),
                                                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                                                }).then(function success(response) {
                                                    //                                                console.log(response.data);
                        ...
                        case "fundgatereferencecheck":
                            //alert("Momo status check started")
                            Swal.fire({
                                focusConfirm: false,
                                title: 'Fundgate Reference Check',
                                //                    input: 'select',
                                //                    inputOptions: options,
                                //                    html: `<input class="swal2-file" id="uploadFile" placeholder="Please select valid Excel File" accept=".xlsx" type="file"/><br /> `,
                                input: 'file',
                                        ...
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'reference').then(function (res) {
                                                ...
                                                return $http({
                                                    ...
                                                    method: 'POST',
                                                    data: $httpParamSerializerJQLike({
                                                        "clientRef": JSON.stringify(res),
                                                        "service": "fundgateReferenceCheck"
                                                    }),
                                                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                                                }).then(function success(response) {
                                                    //                                                console.log(response.data);
                        ...
                        case "billstatuscheck":
                            //alert("Momo status check started")
                            Swal.fire({
                                focusConfirm: false,
                                title: 'Bill Status Check',
                                //                    input: 'select',
                                //                    inputOptions: options,
                                //                    html: `<input class="swal2-file" id="uploadFile" placeholder="Please select valid Excel File" accept=".xlsx" type="file"/><br /> `,
                                input: 'file',
                        ...
                        case "airtimestatuscheck":
                            //alert("Momo status check started")
                            Swal.fire({
                                focusConfirm: false,
                                title: 'Airtime Status Check',
                                //                    input: 'select',
                                //                    inputOptions: options,
                                //                    html: `<input class="swal2-file" id="uploadFile" placeholder="Please select valid Excel File" accept=".xlsx" type="file"/><br /> `,
                                input: 'file',
                        ...
                        case "billandairtimestatuscheck":
                            //alert("Momo status check started")
                            Swal.fire({
                                focusConfirm: false,
                                title: 'Bill and Airtime Status Check',
                                //                    input: 'select',
                                //                    inputOptions: options,
                                //                    html: `<input class="swal2-file" id="uploadFile" placeholder="Please select valid Excel File" accept=".xlsx" type="file"/><br /> `,
                                input: 'file',
                                        ...
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'reference').then(function (res) {
                                                ...
                                                return $http({
                                                    ...
                                                    data: $httpParamSerializerJQLike({
                                                        "uniqueTransId": JSON.stringify(res),
                                                        "service": "billandairtimeStatusCheck"
                                                    }),
                                                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                                                }).then(function success(response) {
                                                    //                                                console.log(response.data);
                        ...
                        case "ghcard":
                            ...
                            Swal.fire({
                                ...
                                //                    input: 'select',
                                //                    inputOptions: options,
                                html: `<div class="ghcard">
                                        <label for="file-upload" class="btn btn-xs sm-btn py-2 px-2 cursor-pointer takePicture" style="background: #3085d6; max-width:120px; display:table; color: #fff; margin-bottom: 20px !important;">Take Picture&nbsp;<i class="fa fa-camera"></i></button></label>
                                        <input style="display:none;" id="file-upload" accept="image/png,image/jpeg,image/jpg,capture=camera" type="file" />
                                        <img id="output"/>                                       
                                        <button class="btn btn-primary btn-xs sm-btn py-2 px-2 cursor-pointer retake" style="background: #3085d6; color: #fff; display:none; margin-bottom: 20px !important;">Retake Picture&nbsp;<i class="fa fa-camera"></i></button></span>
                                        <span class="input-group-btn ghcardnumber" style="text-align:-webkit-center; margin-bottom: 20px !important;">\n<input placeholder="Enter Ghana Card Number. eg. GHA-XXXXXXXXX-X" class="form-control form-control-sm" style="width: 215px;" name="ghcardnumber" id="ghcardnumber" /> </span>
                                       </div>
                                       <div class="ghcardVerified"></div>
                                        <button class="btn btn-primary btn-xs sm-btn py-2 px-2 cursor-pointer newVerify" style="background: #3085d6; color: #fff; display:none; margin-top: 20px !important;">New Verification&nbsp;<i class="fa fa-camera"></i></button></span>`,
//                                type: 'warning',
                                showCancelButton: true,
                                cancelButtonColor: 'red',
                                cancelButtonText: 'Cancel',
                                confirmButtonText: 'Verify',
                                inputPlaceholder: 'Enter Ghana Card Number',
                                allowOutsideClick: false,
                                backdrop: true,
                                showLoaderOnConfirm: true,
                                onOpen: function onOpen(ele) {
                                    $('.swal2-actions').css('display', 'flex');
                                    $('.takePicture').css('display', 'block');

                                    $(ele).find('.ghcard').insertAfter($(ele).find('.swal2-file'));
//                                    var video = document.querySelector("#video");
                                    $('#file-upload').change(function () {
                                        ...
                                        if (file) {
                                            ...
                                            reader.onload = function () {
                                                $("#output").css('display', 'block').attr("src", reader.result);
...
//                                    $('.takePicture, .retake').click(function () {
//                                        base64Image = '';
//                                        if (navigator.mediaDevices.getUserMedia) {
//                                            navigator.mediaDevices.getUserMedia({video: {
//                                                    facingMode: 'environment'
//                                                }}).then(function (stream) {
//                                                        $('#ghcanvas, .retake').css('display', 'none');
//                                                        video.srcObject = stream;
//                                                        $('#ghcontainer, .capture').css('display', 'block');
//                                                        $('.takePicture').css('display', 'none');
//                                                        $('.capture').click(function () {
//                                                            $('#ghcontainer, .capture').css('display', 'none');
//                                                            $('#ghcanvas, .retake').css('display', 'block');
...
//                                    });
                                ...
                                preConfirm: function preConfirm() {
                                    ghNo = document.getElementById('ghcardnumber').value;

                                    if (!ghNo && !base64Image) {
                                        Swal.showValidationMessage('Please enter Ghana Card Number and Take picture');
                                    } else if (!ghNo) {
                                        Swal.showValidationMessage('Please enter Ghana Card Number');
                                    } else if (!base64Image) {
                                        Swal.showValidationMessage('Please Retake Picture');
                                    } else {
//                                        console.log('ghanacard: ', ghNo);
//                                        console.log('base64Image: ', base64Image);
                                        ...
                                        return $http({
                                            ...
                                            var verifiedData = JSON.parse(response.data.data);
                                            var verificationHtml = `<div class="card-container">
                                                                    <span class="pro">VERIFIED</span>
                                                                    <img class="round" src="data:image/png;base64,${verifiedData.biometricFeed.face.data}" alt="user" />
                                                                    <h6><strong>${verifiedData.surname} ${verifiedData.forenames}</strong></h6>
                                                                    <h6>${verifiedData.nationalId}</h6>
                                                                    <p>Date Of Birth: ${verifiedData.birthDate}<br/>Nationality: ${verifiedData.nationality}<br/>Gender: ${verifiedData.gender}</p>
                                                                </div>
                                                                `;
//                                            console.log('face: ', verifiedData.biometricFeed.face.data);
                                            $('.ghcard').css('display', 'none');
                                            $('.newVerify').css('display', 'inline-table');

                                            $('.swal2-actions').css('display', 'none');
                                            $('.ghcardVerified').html(verificationHtml);
                                            $(".swal2-confirm").prop('disabled', true);
                                            $('.newVerify').click(function () {
                                                $('.ghcardVerified, #output, .newVerify').css('display', 'none');
                                                $('.ghcard').css('display', 'block');
                                                ghNo, base64Image = '';
                                                document.getElementById('ghcardnumber').value = '';
                                                $('.swal2-actions').css('display', 'flex');
                                                $('.takePicture').css('display', 'block');
                                            });
                                            return false;
                            ...
                            break;
                        case "smsportalreport":
                            //alert("Sms Portal Report")
                            Swal.fire({
                                focusConfirm: false,
                                title: 'SMS PORTAL',
                                //                    input: 'select',
                                ...
                                preConfirm: function preConfirm(file) {
                                    ...
                                    if (!file) {
                                        ...
                                        var ext = filename.substring(filename.lastIndexOf('.') + 1);
                                        if (ext === "xlsx") {
                                            return uploadFileX($(".swal2-file")[0].files[0], 'destination').then(function (res) {
                                                return $http({
                                                    url: "api/SmsPortalReport",
                                                    method: 'POST',
                                                    data: $httpParamSerializerJQLike({
                                                        "uniqueTransId": JSON.stringify(res),
                                                        "service": "sendsms"
                                ...
                                if (!result.dismiss) {
                                    ...
                                    if (result.code === '00') {
                                        ...
                                        var modelName = "SmsPortalReportTrxs";
                                        var respField = "response_code";
                                        var respMsg = "00";
                                        var amountColumn = "amount";
                                        $scope.pageNumber = 1;
                ...
                $scope.callAutoSwitch = function (dataToExport, route, service) {
                    //alert($scope[dataToExport]);
                    //alert("Autoswitch call started");
                    var json = JSON.stringify($scope[dataToExport]);
                    //console.log(JSON.stringify(json));              
                    if ($scope[dataToExport]) {
                        //alert("Transquery api scope started");
                        //$rootScope.showLoading();
                        var data = $scope[dataToExport].map(function (_ref) {
                            var $$hashKey = _ref.$$hashKey,
                                    item = _objectWithoutProperties(_ref, ["$$hashKey"]);
...
//                            "data": JSON.stringify(data),
                            ...
                            if (resp.code === '00') {
                                ...
                                var modelName = "MobileMoneyReprocessTrxs";
                                var respField = "response_code";
                                var respMsg = "00";
                                var amountColumn = "amount";
                                var feeColumn = "";
...
//                        alert('Connection or Server Error. Try again');
                ...
                $scope.callTransqueryApi = function (dataToExport, route, service) {

                    //alert($scope[dataToExport]);
                    //alert("Transquery api started");
                    var json = JSON.stringify($scope[dataToExport]);
                    //console.log(JSON.stringify(json));              
                    if ($scope[dataToExport]) {
                        //alert("Transquery api scope started");
...
//                        alert('Connection or Server Error. Try again');
                ...
                $scope.callReprocessingTable = function (dataToExport, route, service) {

                    //alert($scope[dataToExport]);
                    //alert("Reprocessing Table call started");
                    var json = JSON.stringify($scope[dataToExport]);
                    //console.log(JSON.stringify(json));              
                    if ($scope[dataToExport]) {
                        //alert("Transquery api scope started");
...
//                        alert('Connection or Server Error. Try again');
                ...
                $scope.updateMobileMoneyTable = function (dataToExport, route, service) {
                    //alert($scope[dataToExport]);
                    //alert("Reprocessing Table call started");
                    var json = JSON.stringify($scope[dataToExport]);
                    //console.log(JSON.stringify(json));              
                    if ($scope[dataToExport]) {
                        //alert("Update Mobile Money table scope started");
                        //$rootScope.showLoading();
                        var data = $scope[dataToExport].map(function (_ref) {
                            var $$hashKey = _ref.$$hashKey,
                                    item = _objectWithoutProperties(_ref, ["$$hashKey"]);
...
//                        alert('Connection or Server Error. Try again');
                ...
                $scope.fgRecordsTotalCredit = 0;
                $scope.fgRecordsTotalDebit = 0;
                ...
                $scope.getSingleCardBalance = function (type) {
                    ...
                    $http.get('api/CardBalance?action=' + type).then(function (response) {
...
//                    console.log('modelname: ', modelName, ' - ', index, ' -- ', $scope[modelName][index]);
                    $scope[modelName][index].checked = !$scope[modelName][index].checked;
//                    console.log('modelname: ', modelName, ' - ', index, ' -- ', $scope[modelName][index].transid, ' - ', $scope[modelName][index].checked);

                    var transId = $scope[modelName][index].transid;
                    if (!$scope[modelName][index].checked) {
                        $scope.selectAll = false;
                    }
//                    console.log('er: ', $scope.selection);
                    if (!$scope[modelName][index].checked) {
                        $scope.selection = $scope.arrayRemove($scope.selection, transId);
                    } else if ($scope[modelName][index].checked && !$scope.selection.includes(transId)) {
                        $scope.selection.push(transId);
                    }
                };

                $scope.toggleSelect = function (modelName, index, type) {
//                    console.log('modelname: ', modelName, ' - ', index, ' -- ', $scope[modelName][index]);
                    $scope[modelName][index].checked = !$scope[modelName][index].checked;
//                    console.log('modelname: ', modelName, ' - ', index, ' -- ', $scope[modelName][index].transid, ' - ', $scope[modelName][index].checked);

                    var transId = $scope[modelName][index][type];
                    if (!$scope[modelName][index].checked) {
                        $scope.selectAll = false;
                    }
//                    console.log('er: ', $scope.selection);
                ...
                $scope.toggleSelectAll = function (modelName) {
                    $scope.$evalAsync(function () {
                        ...
                        for (var i = 0; i < $scope[modelName].length; i++) {
                            ...
                            if (checked && !$scope.selection.includes($scope[modelName][i].transid)) {
                ...
                $scope.toggleSelectAll = function (modelName, key) {
                    $scope.$evalAsync(function () {
                        ...
                        for (var i = 0; i < $scope[modelName].length; i++) {
                            $scope.selectedData[i] = checked;
                            $scope[modelName][i].checked = checked;
                            if (checked && !$scope.selection.includes($scope[modelName][i][key])) {
                                $scope.selection.push($scope[modelName][i][key]);
                            }
                        }
                    });
                    //               console.log('test: '+ $scope.selection);
...
//                                $scope.toggleSelectAll = function (modelName, firstParam, secondParam) {
//                    $scope.$evalAsync(function () {
//                        var checked = $scope.selectAll;
//                        for (var i = 0; i < $scope[modelName].length; i++) {
//                            $scope.selectedData[i] = checked;
//                            $scope[modelName][i].checked = checked;
//                            if (checked && !$scope.selection.includes($scope[modelName][i][firstParam, secondParam])) {
//                                $scope.selection.push($scope[modelName][i][firstParam, secondParam]);
//                            }
//                        }
//                    });
//                    //               console.log('test: '+ $scope.selection);
//                };
                            ...
                            if (paging && paging !== 0) {
                                ...
                                $http({
                                    ...
                                    if (resp.code === "00") {
                                        ...
                                        console.log('data: ' + JSON.stringify(parsedData).CardTransactionsReport);
                                        if (parsedData.CardTransactionsReport) {
                                            $rootScope[modelName] = parsedData.CardTransactionsReport;
                                            $scope.fgRecordsTotalCredit = parsedData.totalCreditAmount;
                                            $scope.fgRecordsTotalDebit = parsedData.totalDebitAmount;

                                        } else {
                                            $rootScope[modelName] = parsedData;
                                        }
                                        $scope.getStatistics(modelName, respField, respMsg, amountColumn, feeColumn);
                                        $scope.begin = parseInt(($scope.pageNumber - 1) * $scope.rowsPerPage);
                                        $scope.end = parseInt($scope.begin + $scope.rowsPerPage);
                ...
                $scope.whitelistblacklistSelected = function (modelName, path, formName, page, paging, respField, respMsg, amountColumn, feeColumn) {
                    //                $('.loader').css('display', 'block');
                    // alert("whitelist started")
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.showLoader = true;
                            $rootScope.showLoading();
...
//                                    //                        console.log('field:  $scope[filtered' + modelName + ']' + JSON.stringify($scope['filtered' + modelName]));
                                    ...
                                    if (response.data.code === '00') {
                                        $(buttonId).html('OPERATION SUCCESSFUL');
                                        Swal.fire({
                                            position: 'top-end',
                                            type: 'success',
                                        ...
                                        var modelName = "WhitelistBlacklistTrxs";
                                        var respField = "response_code";
                                        var respMsg = "00";
                                        var amountColumn = "amount";
                                        $scope.pageNumber = 1;
//                            $rootScope[modelName] = JSON.parse(result.data);
                                        ...
                                        $rootScope.$apply($rootScope[modelName] = JSON.parse(response.data));
                                        $scope.$apply($scope.getStatistics(modelName, respField, respMsg, amountColumn));
                                        $scope.$apply($scope.begin = parseInt(($scope.pageNumber - 1) * $scope.rowsPerPage));
                                        $scope.$apply($scope.end = parseInt($scope.begin + $scope.rowsPerPage));
                                        $rootScope.$apply($rootScope['filtered' + modelName] = $rootScope[modelName].slice($scope.begin, $scope.end));
                ...
                $scope.fundstransferSelected = function (modelName, path, formName, page, paging, respField, respMsg, amountColumn, feeColumn) {
                    //                $('.loader').css('display', 'block');
                    //alert("funds transfer stated started")
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.showLoader = true;
                            $rootScope.showLoading();
...
//                            $rootScope[modelName] = JSON.parse(result.data);
                ...
                $scope.getAllRecords = function (modelName, path, formName, page, paging, respField, respMsg, amountColumn, feeColumn) {
                    //                $('.loader').css('display', 'block');
                    //alert("get all transactions started")
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.showLoader = true;
                            $rootScope.showLoading();
...
//                                        "appId": app_id,
//                                        "type": type,
                                        "service": "fundstransfertransactionsall"
                                    }),
                                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                                }).then(function success(response) {
//                                    console.log(response.data);
                                    ...
                                    if (resp.code === "00") {
                                        ...
                                        console.log("parsed Data 2" + JSON.stringify(parsedData));
                                        //console.log('data: ' + JSON.stringify(parsedData).CardTransactionsReport);
                                        $rootScope[modelName] = JSON.stringify(parsedData);
                                        $scope.getStatistics(modelName, respField, respMsg, amountColumn, feeColumn);
                                        $scope.begin = parseInt(($scope.pageNumber - 1) * $scope.rowsPerPage);
                                        $scope.end = parseInt($scope.begin + $scope.rowsPerPage);
                                        $rootScope['filtered' + modelName] = $scope[modelName].slice($scope.begin, $scope.end);
                ...
                $scope.initializeSettings = function (path, action, modelName, type, callBack, callbackParam) {
                ...
                $scope.checkReversalStatus = function (reference) {
                    $scope.ref = reference;
                    $scope.showLoader = true;
                    $rootScope.showLoading();
                    $('.reload').addClass('fa-spin text-info');
                ...
                $.contextMenu({
                    ...
                    //                autoHide: true,
                    build: function build(triggerElement, e) {
                ...
                $scope.processRequest = function (request, action, type) {
                    if (!type) {
                        type = 'transid';
                    }
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            ...
                            if ($scope.selection.length > 0) {
                                Swal.fire({
                                    title: "Are you sure you want to " + action.toUpperCase() + " this ?",
//                    text: "This will " + action + " " + $scope.username,
                                    html: "<b>This action is IRREVERSIBLE</b>",
                                    type: 'warning',
                                    showCancelButton: true,
//                            showDenyButton: true,
                                    cancelButtonText: 'CANCEL',
                                    confirmButtonColor: '#12c637',
                                    confirmButtonText: action.toUpperCase(),
//                            denyButtonText: 'DENY',
                                    cancelButtonColor: "#e42c2c",
                                    showLoaderOnConfirm: true,
                                    allowOutsideClick: function allowOutsideClick() {
                                        return !Swal.isLoading();
                                    },
...
//                                    console.log('RESULT: ' + JSON.stringify(result));ss
                                    $scope.selectAll = false;
                                    var modelName = "ErequestTrxs";
                                    $scope.toggleSelectAll(modelName);
                                    $scope.selectedData = [];
                                    $scope.selection = [];
                                    //do asyc
                                    if (result.value) {
                                        var respTitle = result.value.data.message.toUpperCase();
                                        var respText = (result.value.data.code === '00' ? 'Your request will be processed.' : 'An Error Occured processing your requested');
                                        var respType = (result.value.data.code === '00' ? 'success' : 'error');
                                        if (result.value.data.code === '00') {

                                            var resp = result.value.data.data;
                                            var modelData = $rootScope[modelName];
                                            $rootScope[modelName] = "";

                                            var newData = JSON.parse(resp);
//                                $rootScope[modelName] = [...modelData, ...(JSON.parse(resp.data))];
//                                            var ids = new Set(newData.map(d => d[type]));
//                                            $rootScope[modelName] = [...modelData.filter(d => !ids.has(d[type])), ...newData];

                                            $rootScope[modelName] = modelData.map(obj => newData.find(o => o[type] === obj[type]) || obj);
                                            console.log(modelData.map(obj => newData.find(o => o[type] === obj[type]) || obj));
                                            // $rootScope[modelName] = newData.reduce((res, item) => res.filter(i => i[type] !== item[type]).concat(item), modelData);
                ...
                $scope.reverseSelected = function (type, choice, reversalType) {
                    if ($scope.selection.length > 0) {
                        var buttonId = '#reverse-btn';
                        $rootScope.showLoading();
                        $(buttonId).prop("disabled", true);
                        ...
                        $http({
                            ...
                            if (type === "ini") {
                                ...
                                if (resp.code === "00") {
                                    //                            console.log('data: ' + JSON.stringify(resp));
                                    var newData = JSON.parse(resp.data);
                                    //                                $rootScope[modelName] = [...modelData, ...(JSON.parse(resp.data))];

                                    $rootScope[modelName] = newData.reduce((res, item) => res.filter(i => i.transid !== item.transid).concat(item), modelData);
                                    $scope.getStatistics(modelName);
                                    $scope.begin = parseInt(($scope.pageNumber - 1) * $scope.rowsPerPage);
                                    $scope.end = parseInt($scope.begin + $scope.rowsPerPage);
                                    $rootScope['filtered' + modelName] = $scope[modelName].slice($scope.begin, $scope.end);
                ...
                $scope.reprocessSelected = function (type, choice, reprocessType) {
                    if ($scope.selection.length > 0) {
                        var buttonId = '#reprocess-btn';
                        $rootScope.showLoading();
                        $(buttonId).prop("disabled", true);
                        if (type === "ini") {
                            $(buttonId).html("<i class='fa fa-spinner fa-spin btn-loader'></i> Saving...");
                        ...
                        $http({
                            ...
                            data: $httpParamSerializerJQLike({
                                "transid": JSON.stringify($scope.selection),
                                "choice": choice,
                                "transtype": reprocessType,
                                "service": type === 'auth' ? 'authorizeReprocess' : 'initiateReprocess'
                            }),
                            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                        }).then(function success(response) {
                            $scope.showLoader = false;
                            ...
                            if (type === "ini") {
                                ...
                                if (resp.code === "00") {
                                    ...
                                    $rootScope[modelName] = newData.reduce((res, item) => res.filter(i => i.reference !== item.reference).concat(item), modelData);
                                    $scope.getStatistics(modelName);
                                    $scope.begin = parseInt(($scope.pageNumber - 1) * $scope.rowsPerPage);
                                    $scope.end = parseInt($scope.begin + $scope.rowsPerPage);
                                    $rootScope['filtered' + modelName] = $scope[modelName].slice($scope.begin, $scope.end);
                ...
                $scope.momoReprocessSelected = function (type, choice, reversalType) {
                    alert("Momo reprocess clicked");
                    if ($scope.selection.length > 0) {
                        var buttonId = '#reverse-btn';
                        $rootScope.showLoading();
                        $(buttonId).prop("disabled", true);
                ...
                $scope.nlaReversalSelected = function (action, dataToExport) {
                    //alert("NLA reversal select started")
                    //alert($scope[dataToExport])
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.hideFilterBox();
                            ...
                            if ($scope.selection.length > 0) {
                                ...
                                Swal.fire({
                                    ...
                                    preConfirm: function preConfirm() {
                                        ...
                                        $(buttonId).html("<i class='fa fa-spinner fa-spin btn-loader'></i> Initiating...");

                                        return $http({
                                            url: 'api/nlaliquidationreport',
                                            method: 'POST',
                                            data: $httpParamSerializerJQLike({
                                                "transid": JSON.stringify($scope.selection),
                                                //"subscriber_id": JSON.stringify($scope.selection),
                                                "data": JSON.stringify($scope[dataToExport]),
                                                "choice": action,
...
//                                        Swal.fire({title: respTitle, text: respText, type: respType, timer: 1500});
                                    ...
                                    if (result.value.data.code === '00') {
                                        $(buttonId).html('INITIATE SUCCESSFUL');
                                        Swal.fire({
                                            position: 'top-end',
                                            type: 'success',
                                            title: result.value.data.data,
                ...
                $scope.nlaAuthorizeSelected = function (action, dataToExport) {
                    //alert("NLA authorise select started")
                    //alert($scope[dataToExport])
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.hideFilterBox();
...
//                                        Swal.fire({title: respTitle, text: respText, type: respType, timer: 1500});
                                    ...
                                    if (result.value.data.code === '00') {
                                        $(buttonId).html('AUTHORISE SUCCESSFUL');
                                        Swal.fire({
                                            position: 'top-end',
                                            type: 'success',
                                            title: result.value.data.data,
                ...
                $scope.nlaDenySelected = function (action, dataToExport) {
                    //alert("NLA authorise select started")
                    //alert($scope[dataToExport])
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
...
//                                        Swal.fire({title: respTitle, text: respText, type: respType, timer: 1500});
                ...
                $scope.vasReprocessInitiate = function (action, dataToExport) {
                    //alert("VAS reprocess select started")
                    //alert($scope[dataToExport])
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.hideFilterBox();
...
//                                        Swal.fire({title: respTitle, text: respText, type: respType, timer: 1500});
                ...
                $scope.vasReprocessAuthorizeSelected = function (action, dataToExport) {
                    //alert("Vas authorise select started")
                    //alert($scope[dataToExport])
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.hideFilterBox();
                            ...
                            if ($scope.selection.length > 0) {
                                ...
                                Swal.fire({
                                    ...
                                    preConfirm: function preConfirm() {
                                        $(buttonId).html("<i class='fa fa-spinner fa-spin btn-loader'></i> Authorizing...");
                                        return $http({
                                            url: 'api/VasgateReprocess',
                                            method: 'POST',
                                            data: $httpParamSerializerJQLike({
//                                                "transid": JSON.stringify($scope.selection), 
                                                //"original_reference": JSON.stringify($scope.selection),                                                
//                                                "data": JSON.stringify($scope[dataToExport]),
                                                "data": JSON.stringify($scope.selection),
                                                "choice": action,
                                                "service": 'authorizeVasReprocess'
                                                        //"service": type === 'auth' ? 'authorizeReversal' : 'initiateReversal'
                                            }),
                                            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                                        }).then(function success(response) {
...
//                                        Swal.fire({title: respTitle, text: respText, type: respType, timer: 1500});
                ...
                $scope.fundstransferAuthorizeSelected = function (action, dataToExport) {
                    //alert("NLA authorise select started")
                    //alert($scope[dataToExport])
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            ...
                            if ($scope.selection.length > 0) {
                                ...
                                Swal.fire({
                                    ...
                                    //                    text: "This will " + action + " " + $scope.username,
                                    html: "<b>This action is IRREVERSIBLE</b>" +
                                            '<p>Enter Pin: <input type="password" id="swal-pin" class="swal2-input"></p>',
...
//                                        Swal.fire({title: respTitle, text: respText, type: respType, timer: 1500});
                ...
                $scope.sendBulkSmsSelected = function (action, dataToExport) {
                    //alert("Send sms selected")
                    //alert($scope[dataToExport]);
                    console.log($scope[dataToExport])
                    $scope.$evalAsync(function () {
                        $rootScope.$evalAsync(function () {
                            $scope.hideFilterBox();
                            console.log('selection: ', $scope.selection);
...
//                                    if (result.value.data.code === '00') {
//                                       //if (result.code === '00') {
//                                        $(buttonId).html('SENDING SUCCESSFUL');
//                                        Swal.fire({
//                                            position: 'top-end',
//                                            type: 'success',
//                                            title: result.value.data.data,
...
//                                    }
                ...
                $scope.initializeProviders = function () {
                    $http.get('api/MerchantsInfo?action=providers').then(function (response) {
                        $scope.providerList = response.data;
                    });
                };
                $scope.initializeMobileAppIdList = function () {
                    $http.get('api/MerchantsInfo?action=appIdList').then(function (response) {
                        $scope.appIdList = response.data;
                    });
                };
                $scope.initializeSenderIds = function () {
                    $http.get('api/MerchantsInfo?action=senderIds').then(function (response) {
                        $scope.providerList = response.data;
                    });
                };
                $scope.initializeBanks = function () {
                    $http.get('api/MerchantsInfo?action=banks').then(function (response) {
                        $rootScope.banksList = response.data;
                    });
                };
                $scope.initializeChannels = function () {
                    $http.get('api/MerchantsInfo?action=channels').then(function (response) {
                        $scope.channelsList = response.data;
                    });
                };
                $scope.initializeVasGateMerchants = function () {
                    $http.get('api/MerchantsInfo?action=vasgate').then(function (response) {
                        $scope.vasgateMerchantList = response.data;
                    });
                };
                $scope.initializeTMCSettings = function () {
                    $scope.record = "new";
                    $http.get('api/MerchantsInfo?action=bankNodes').then(function (response) {
                ...
                $scope.initializeWCMerchants = function () {
                    //                console.log('log');
                    $http.get('api/MerchantsInfo?action=wcMerchants').then(function (response) {
                ...
                $scope.initializeUserTypes = function () {
                    $http.get('api/MerchantsInfo?action=typeIdList').then(function (response) {
                ...
                $scope.initializeBucketBalance = function () {
                    $http.get('api/WalletBalance?action=fetchBuckets').then(function (response) {
                ...
                $scope.fetchBucketBalance = function () {
                    angular.forEach($scope.bucketBalanceTypes, function (value, index) {
                        $http.get('api/WalletBalance?action=fetchBucketsBalance&type=' + value.alias).then(function (response) {
                ...
                $rootScope.getUserTypeOptions = function (typeId, scopeVariable) {
                    ...
                    $http.get('api/MerchantsInfo?action=typeIdListOptions&option=' + typeId).then(function (response) {
                ...
                $scope.initSenderIdList = function () {
                    $http.get('api/MerchantsInfo?action=senderIdList').then(function (response) {
                        //                            var model = $parse(scopeVariable);
                ...
                $scope.initProviderList = function () {
                    $http.get('api/MerchantsInfo?action=providerList').then(function (response) {
                        //                            var model = $parse(scopeVariable);
                        //                            model.assign($scope, response.data);
                ...
                $scope.getCPayMerchants = function () {
                    $http.get('api/MerchantsInfo?action=cpayMerchants').then(function (response) {
                        $scope.cpayMerchantList = response.data;
                    });
                };
                $scope.getScbMerchants = function () {
                    $http.get('api/MerchantsInfo?action=scbMerchants').then(function (response) {
                        $scope.scbMerchantList = response.data;
                    });
                };
                $scope.getEtzMerchants = function () {
                    $http.get('api/MerchantsInfo?action=etzMerchants').then(function (response) {
                        $scope.etzMerchantList = response.data;
                    });
                };
                $scope.getEtzMerchantProducts = function () {
                    $http.get('api/MerchantsInfo?action=etzMerchantProducts').then(function (response) {
                        $scope.etzMerchantProductList = response.data;
                    });
                };
                $scope.getMPayProducts = function (init) {
                    $http.get('api/MerchantsInfo?action=mpayProducts&merchant=' + $scope.merchant + (init !== undefined ? "&init=" + init : "")).then(function (response) {
                ...
                $scope.getMPayMerchants = function () {
                    ...
                    $http.get('api/MerchantsInfo?action=' + ($rootScope.testEtz() ? $scope.bank : j.split(":")[1]).toLowerCase() + 'Merchants').then(function (response) {
                ...
                $scope.updateAMLData = function (index) {
                    //alert("AML update started")
                    $scope.formName = "Update Action";
                    $scope.formType = "UPDATE";
                    $scope.formButton = "Modify Action!";
                    var action = $scope.filteredWhitelistBlacklistTrxs[index].action;
                    var account = $scope.filteredWhitelistBlacklistTrxs[index].account;
...
//                    var type_2 = $scope.filteredPortalSettings[index].type_2;
                    ...
                    var modifyHtml = "<div class=\"card-block font08\">\n                                    <br/>\n                                        <form method=\"post\" id=\"amlUpdateForm\">\n                                            <table class=\"table borderless\" id=\"item_table\">        \n                                                <tr>\n                                                    <td><label for=\"name\" style=\"font-size: 0.763em; margin-bottom: 0em !important;\">Account:<i class=\"text-danger\" style=\"display: inline-block;\"> *</i></label></td>\n                                                    <td><input class=\"form-control form-control-sm text-primary\" name=\"name\"  ng-model=\"name\" id=\"name\" value=\"" + account + "\" disabled required/></td>\n                                                </tr>\n                                        <tr>\n                                                    <td><label for=\"name\" style=\"font-size: 0.763em; margin-bottom: 0em !important;\">Action:<i class=\"text-danger\" style=\"display: inline-block;\"> *</i></label></td>\n                                                    <td><input class=\"form-control form-control-sm text-primary\" name=\"name\"  ng-model=\"name\" id=\"name\" value=\"" + newAction + "\" disabled required/></td>\n                                                </tr>\n                                                                                                                                                     </table>\n                                            <input type=\"hidden\" name=\"action\" value=\"" + newAction + "\"/>\n           <input type=\"hidden\" name=\"account\" value=\"" + account + "\"/>\n                                                                          </form>\n                                    </div>";
                    Swal.fire({
                        //                            title: $scope.formName,
                        html: modifyHtml,
                        showCancelButton: true,
                        ...
                        preConfirm: function preConfirm(login) {
                            ...
                            //                            }
                            //                        });
                            var loginForm = $('#amlUpdateForm').serializeJSON({checkboxUncheckedValue: "false"});
                            console.log('JSON: ' + JSON.stringify(loginForm));
                            return $http({
                                //url: $scope.adminProxy,
                                url: 'api/whitelistblacklistinsert',
                                method: 'POST',
                                data: $httpParamSerializerJQLike({
//                                    "modifyAmlPortal": JSON.stringify(loginForm),
                                    "data": JSON.stringify(loginForm),
                                    "service": "amlPortalUpdate"
                                }),
                                headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                            }).then(function success(response) {
//                                console.log('response.data' + response.data.data);
...
//                                        //title: response.data.data,
//                                        title: response.data.data,
//                                      //title: JSON.parse(response.data.data),
...
//                        }
                ...
                $.contextMenu({
                    ...
                    build: function build(triggerElement, e) {
                        ...
                        }, _defineProperty(_menuData, statusKey, {"name": statusText, "icon": statusIcon}), _defineProperty(_menuData, "resetPassword", {"name": "Password Reset", "icon": "user-resetpassword"}), _defineProperty(_menuData, "auditTrail", {"name": "View Audit Trail", "icon": "audit-trail"}), _menuData);
                        return {
                            callback: function callback(key, options) {
                                //                            window.console && console.log(m);
                ...
                $.contextMenu({
                    ...
                    build: function build(triggerElement, e) {
                        ...
                        bank = $scope.filteredMobileAppUsers[$scope.userIndex].bankCode;
                        //                        });
                        //                    var statusIcon = "";
                        //                    if (status === 'ACTIVE') {
                        ...
                        if (bank === '005') {
                            if (status === 'BLOCKED') {
                                ...
                                menuData = (_menuData3 = {}, _defineProperty(_menuData3, statusKey2, {"name": statusText2, "icon": statusIcon2}), _defineProperty(_menuData3, "resetPassword", {"name": "Password Reset", "icon": "user-resetpassword"}), _defineProperty(_menuData3, "resetProfile", {"name": "Reset Profile", "icon": "user-resetpassword"}), _defineProperty(_menuData3, "resetPin", {"name": "Reset Pin", "icon": "user-resetpassword"}), _defineProperty(_menuData3, "clearProfile", {"name": "Clear Profile", "icon": "user-resetpassword"}), _menuData3);
                            } else {
                                var _menuData4;

                                menuData = (_menuData4 = {}, _defineProperty(_menuData4, statusKey, {"name": statusText, "icon": statusIcon}), _defineProperty(_menuData4, "blockUser", {"name": "Block User", "icon": "user-ban"}), _defineProperty(_menuData4, "resetPassword", {"name": "Password Reset", "icon": "user-resetpassword"}), _defineProperty(_menuData4, "resetProfile", {"name": "Reset Profile", "icon": "user-resetpassword"}), _defineProperty(_menuData4, "resetPin", {"name": "Reset Pin", "icon": "user-resetpassword"}), _defineProperty(_menuData4, "clearProfile", {"name": "Clear Profile", "icon": "user-resetpassword"}), _menuData4);
                            }
                        } else {
                            if (status === 'BLOCKED') {
                                var _menuData3;
...
//                        var statusKey = status === '1' ? "disableSetting" : "enableSetting";
                        var menuData = _defineProperty({
//                            "editAMLAction": {"name": "Edit Action", "icon": "edit"}
                            "editAMLAction": {"name": "Edit Action"}
                        });
                        return {
                            callback: function callback(key, options) {
                                //                            window.console && console.log(m);
...
//                                }
                                ...
                                //$scope.portalUpdateForm(settingIndex);
                                //alert("AML record started")
                                $scope.updateAMLData(settingIndex);
                                //$scope.updateAMLData(key, $scope.filtereduserProfiles[$scope.userIndex].username);
                ...
                $scope.changeMobileAppStatus = function (index, action) {
                    ...
                    $scope.appId = $scope.filteredMobileAppUsers[index].appId;
                    ...
                    $rootScope.$evalAsync(function () {
                        ...
                        if (ele) {
                            ...
                            for (var i = 0; i < $rootScope.banksList.length; i++) {
                                ...
                                html += '<option value="' + $rootScope.banksList[i]['issuer_code'] + '" ' + ($rootScope.banksList[i]['issuer_code'] === usercode ? 'selected' : '') + ' >' + $rootScope.banksList[i]['issuer_name'] + '</option>';
                            }
                ...
                $scope.populateBranchSelect = function (usercode) {
                    console.log('herer: ', usercode, ' -- ', $rootScope.branchList);
                    if (!usercode) {
                        usercode = 'ALL';
                    }
                    ...
                    $scope.$evalAsync(function () {
                        ...
                        if (ele) {
                            var html = "";
                            html += '<option value="ALL" ' + ('ALL' === usercode ? 'selected' : '') + '>ALL</option>';
                            for (var i = 0; i < $rootScope.branchList.length; i++) {
                                // POPULATE SELECT ELEMENT WITH JSON.
                                //                        console.log($rootScope.banksList[i]['issuer_code']);

                                html += '<option value="' + $rootScope.branchList[i]['issuer_code'] + '" ' + ($rootScope.branchList[i]['issuer_code'] === usercode ? 'selected' : '') + ' >' + $rootScope.branchList[i]['issuer_name'] + '</option>';
                            }
                            //                    console.log('html: ' + html);
                            ele.innerHTML = html;
                        }
                    ...
                    $.confirm({
                        ...
                        content: function content() {
                            ...
                            return $.ajax({
                                ...
                                self.setContent('' + '<form ng-submit="updateCard();">' + '<div class="row" data-ng-init="CheckId()">' + '   <div class="col-9">' + '      <table class="table borderless">    ' + '         <tr>' + '            <td><label>Card Number:</label></td>' + '          <td><label class="">&nbsp;&nbsp;&nbsp;' + $scope.card_num3 + '</label></td>' + '     </tr>' + '    <tr>' + '       <td><label for="firstName">First Name:</label></td>' + '      <td><label class="">&nbsp;&nbsp;&nbsp;' + $scope.firstName + '</label></td>' + ' </tr>' + '<tr>' + '   <td><label for="lastName">Last Name:</label></td>' + '      <td><label class="">&nbsp;&nbsp;&nbsp;' + $scope.lastName + '</label></td>' + '</tr>' + '<tr>' + '   <td><label for="from_source">Mobile No:</label></td>' + '      <td><label class="">&nbsp;&nbsp;&nbsp;' + $scope.from_source2 + '</label></td>' + '</tr> ' + '<tr>' + '  <td><label for="reason">Reason:</label></td>' + '  <td><textarea class="form-control form-control-sm text-primary" name="reason" ng-model="reason" id="reason" max-rows="3" style="resize:none;" disabled> ' + $scope.reason + '</textarea></td>' + '</tr>' + '<tr>' + '<td><label for="fax">ID:</label></td>' + '<td>' + '<div class="input-group">' + '<input type="text" class="form-control form-control-sm text-primary" value="' + $scope.fax + '" disabled>' + '<span class="input-group-btn">' + ' <button class="btn btn-success btn-sm">' + '   Verify' + '</button>' + '</span>' + '</div>' + '</td>' + '</tr>' + '</table>' + ' </div>' + ' <div class="col-3">' + '<img id="verifyIDimage" src="http://ssl.gstatic.com/accounts/ui/avatar_2x.png" class="avatar img-circle img-thumbnail" alt="avatar">' + ' </div>' + '</div>' +
                                        //                    '<div class="row">' +
                                        //                    '<div class="col col-6">' +
                ...
                $scope.showAccountUserForm = function (profile) {
                    ...
                    $scope.account_number = 'N/A';
                    $http.get('api/MobileAppMgmt?action=getUserAccounts&mobile_no=' + profile.Phone + '&appId=' + profile.AppId + '&bank_code=' + profile.issuer_code + '&type=1&card_num=' + profile.card_num).then(function success(response) {
                        angular.forEach(response.data, function (value, index) {
                            $('#account_number').text(value.id);
                            $scope.account_number = value.id;
                        });
                    }, function error(response) { });
                ...
                $scope.showUserAccounts = function (profile, type, id) {
                    ...
                    if (type === 2) {
                        ...
                        appId = profile.AppId;
                        bankCode = profile.issuer_code;
                        customerId = profile.Client_ID;
                        cardNumber = profile.card_num;
                    }

                    $('#account' + id).html("<i class='fa fa-spinner fa-spin btn-loader'></i> Checking...");
                    $http.get('api/MobileAppMgmt?action=getUserAccounts&mobile_no=' + mobileNo + '&appId=' + appId + '&bank_code=' + bankCode + '&type=' + type + '&card_num=' + cardNumber).then(function success(response) {
//                        var resp = response.data;
                        $('#account' + id).html('<i class="fa fa-eye"></i> &nbsp; SHOW');

                        var options = "";
                        angular.forEach(response.data, function (value, index) {
                            options += '<p>' + value.id + '</p>';
                        });
                        if (options.length === 0) {
                            options = "<p>N/A</p>";
                        }
                        Swal.fire({title: 'Accounts Data', html: options, type: 'info', allowOutsideClick: false});
                    }, function error(response) {
                        $('#account' + id).html('<i class="fa fa-eye"></i> &nbsp; SHOW');
                    });
                ...
                $scope.hideMyFilterBox = function () {
                    $scope.showTrxFilterBox = false;
                    $scope.showMyFilterBox = false;
                    //                $('.filter-box').toggleClass('slideOutRight slideInRight');
                    $(".my-filter-box").css("display", "none");
                };
                ...
                $scope.showMyFilterBoxView = function () {
                    $scope.showTrxFilterBox = true;
                    $scope.showMyFilterBox = true;
                    $(".my-filter-box").css("display", "block");
                };
                ...
                //openMyFilterBox
                $scope.openMyFilterBoxView = function () {
                    //                console.log('sdfsdf');
                    //alert("Filter box");
                    if ($scope.showMyFilterBox) {
                        $scope.hideMyFilterBox();
                    } else {
                        $scope.showMyFilterBoxView();
                    }
                };
                ...
                $scope.genRowColor = function (str) {
                    ...
                    if (rowClass !== "0" && rowClass !== "00" && rowClass !== "NULL") {
                        if (rowClass === '05' || rowClass === '31') {
                            return "blue";
                        }
                        return "red";
                    }
...
//                    }
                    ...
                    if (str2 !== '00' && str2 !== '0') {
                        if (!str2 || str2 === '58' || str2 === '05' || str2 === '66' || str2 === null || str2 === '') {
                            return "blue";
                        }
                        if (str !== "0") {
                            if (str === '31') {
