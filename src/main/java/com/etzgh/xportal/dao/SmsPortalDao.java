...
//        new SmsPortalDao().getNlaLiquidationReport(r);
    }
    ...
    public List<Sms> getSmsUploadFileData(String jsonString) {
      //public String getSmsUploadFileData(String jsonString) {
        ...
        List<Sms> records = new ArrayList<>();
        ...
        List<Sms> recordsList = gson.fromJson(data, new TypeToken<List<Sms>>() {
        }.getType());

        recordsList.stream().forEach(f -> {
            if (f.getDestination() != null && !f.getDestination().trim().isEmpty()) {
                //references.add(f.getReference().trim());
                System.out.println("Destination from excel file" + f.getDestination());
                ...
                try {
                     ...
                     Sms sms = new Sms();
//                    String response = sendSms(f.getDestination(), f.getMessage());
//                    System.out.println("Ending send");
//
...
//
//                        Sms smsResponse = j.fromJson(response, Sms.class);
//
//                        String provider = smsResponse.getProvider();
//                        String messageId = smsResponse.getMessageId();
//                        String message = smsResponse.getMessage();
//                        String msisdn = smsResponse.getMsisdn();
//                        String providerResponse = smsResponse.getProviderResponse();
//                        String responseCode = smsResponse.getResponseCode();
//                       
//
...
//                          sms.setProviderResponse(providerResponse);
//                          sms.setResponseCode(responseCode);
//
//                    }

                      sms.setMessage(f.getMessage());
                      sms.setDestination( f.getDestination());
...
//                    log.error(ERROR, e);
        ...
        public List<Sms> getSendBulkSms(String jsonString) {
      //public String getSmsUploadFileData(String jsonString) {

        log.info("Send Bulk Sms request received >> {0}" + jsonString);
        Gson j = new Gson();
        ...
        List<Sms> recordsList = gson.fromJson(data, new TypeToken<List<Sms>>() {
                ...
                List<String> selectedList = gson.fromJson(transid, new TypeToken<List<String>>() {
        }.getType());
...
//                System.out.println("Message from excel file" + f.getMessage());
//
//                System.out.println("Calling send bulk sms");
                
                
                for(int i=0; i<selectedList.size(); i++){
                    
                     System.out.println("Starting selected check");
                    
                    if(f.getDestination().trim().equals(selectedList.get(i))){
...
//                }
    ...
    public String sendSms(String destination, String message) {

        String smsurl = config.getProperty("smsurl");
        String from = config.getProperty("from");
        String type = config.getProperty("type");
        String terminalId = config.getProperty("terminalId");  
        String res = "";
        ...
        try {
            ...
            System.out.println("sms request"+obj.toString());
//            String res = new DoHTTPRequest().post(smsurl, obj.toString());
             res = new DoHTTPRequest().post(smsurl, obj.toString());

            // res = "";
            log.info("Sms url:: " + smsurl);

            log.info("sms  RESP " + res);
