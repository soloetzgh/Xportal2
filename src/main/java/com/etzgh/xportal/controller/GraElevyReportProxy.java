...
import com.etzgh.xportal.dao.GraElevyDao;
 ...
 */
public class GraElevyReportProxy extends HttpServlet {
    ...
    private static final Logger log = Logger.getLogger(GraElevyReportProxy.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, ParseException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            apiData.setTransactionStatus(request.getParameter("transactionStatus"));
            ...
            Boolean allowed = (!apiData.getType_id().isEmpty() && (apiData.getType_id().contains("[0]") || (apiData.getType_id().contains("[90]"))));
...
//            }
            if (allowed) {
                ...
                if (apiData.getService().equalsIgnoreCase("channels")) {
                    ...
                    channels = null;
                } else if (apiData.getService().equalsIgnoreCase("graelevytransreport")) {
                    System.out.println("Gra Elevy transactions started");
                    if (allowed) {
                        List<GraElevyReport> records = new GraElevyDao().getGraElevyTransaction(new Gson().toJson(apiData));
