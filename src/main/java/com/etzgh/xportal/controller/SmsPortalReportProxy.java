...
import com.etzgh.xportal.dao.SmsPortalDao;
 ...
 */
public class SmsPortalReportProxy extends HttpServlet{
   ...
   private static List<Object[]> noderecords;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, ParseException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //local
//            if (userType.contains("[0]") || userType.contains("[85]")) {
//                String userRoles = new UserProfileService().getRoleParameters("[2500000000000069]",
...
//            }
               ...
               //live
            if (userType.contains("[0]") || userType.contains("[87]")) {
                String userRoles = new UserProfileService().getRoleParameters("[2500000000000068]",
                        userSession.getUser().getUser_code());
                log.info("usrRoles: " + userRoles);
                roleList = Arrays.asList(userRoles.split("~"));
                log.info("roleList: " + roleList);
            ...
            if (allowUpdate) {
                ...
                if (apiData.getService().equalsIgnoreCase("channels")) {
                    ...
                    channels = null;
                } else if (apiData.getService().equalsIgnoreCase("sendsms")) {
                    System.out.println("Sms Proxy started");
                    if (allowUpdate) {
                        List<Sms> records = new SmsPortalDao().getSmsUploadFileData(new Gson().toJson(apiData));
                ...
                }else if (apiData.getService().equalsIgnoreCase("sendbulksms")) {
                    System.out.println("Send Bulk Sms Proxy started");
                    if (allowUpdate) {
                        List<Sms> records = new SmsPortalDao().getSendBulkSms(new Gson().toJson(apiData));
