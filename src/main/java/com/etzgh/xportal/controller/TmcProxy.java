...
import com.etzgh.xportal.dao.TmcDao;
 ...
 */
public class TmcProxy extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (allowed) {
                ...
                if (apiData.getService().equalsIgnoreCase("transactions") || apiData.getService().equalsIgnoreCase("search")) {
                    ...
                    List<TMC> records = new TmcDao().getTmcTransactions(new Gson().toJson(apiData));
                    ...
                    out.println(new Gson().toJson(channels));
                    channels = null;
                } else if (apiData.getService().equalsIgnoreCase("tmcupdate")) {
                    if (allowUpdate) {
                        List<TMC> records = new TmcDao().updateTmcStatus(new Gson().toJson(apiData));
                    ...
                    if (viewUpdate) {
                        List<TmcUpdate> records = new TmcDao().getTmcUpdateLog(new Gson().toJson(apiData));
