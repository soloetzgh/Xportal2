...
import com.etzgh.xportal.dao.FundsTransferDao;
 ...
 */
public class FundsTransferProxy extends HttpServlet{
    
        private static List<Object[]> noderecords;

    private static final GeneralUtility utility = new GeneralUtility();

    private static final FundsTransferDao vasreprocess = new FundsTransferDao();
    private static final AppDao ad = new AppDao();
    private static final Logger log = Logger.getLogger(FundsTransferProxy.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, ParseException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //start
            //String newUserCompany = userSession.getUser().
            //end
            ...
            apiData.setSourceCard(request.getParameter("sourceCard"));
            apiData.setDestinationAccount(request.getParameter("destinationAccount"));
            apiData.setDestinationBank(request.getParameter("destinationBank"));
            apiData.setNarration(request.getParameter("narration"));
            apiData.setTransactionReference(request.getParameter("transactionReference"));
            apiData.setPin(request.getParameter("pin"));
            apiData.setNetwork(request.getParameter("network"));
            ...
            //local
            if (userType.contains("[0]") || userType.contains("[82]") || userType.contains("[83]")) {
                ...
                allowUpdate = !roleList.isEmpty();
                            ...
                            String companyId = new UserProfileService().getRoleParameters("[2500000000000066]",
                        userSession.getUser().getUser_code());
            
                        //demo
//            if (userType.contains("[0]") || userType.contains("[83]") || userType.contains("[84]")) {
//                String userRoles = new UserProfileService().getRoleParameters("[2500000000000067]",
//                        userSession.getUser().getUser_code());
//                log.info("usrRoles: " + userRoles);
//                roleList = Arrays.asList(userRoles.split("~"));
//                log.info("roleList: " + roleList);
...
//            }
//            
//                                        String companyId = new UserProfileService().getRoleParameters("[2500000000000068]",
//                        userSession.getUser().getUser_code());

                              //live
//            if (userType.contains("[0]") || userType.contains("[85]") || userType.contains("[86]")) {
//                String userRoles = new UserProfileService().getRoleParameters("[2500000000000066]",
//                        userSession.getUser().getUser_code());
//                log.info("usrRoles: " + userRoles);
//                roleList = Arrays.asList(userRoles.split("~"));
//                log.info("roleList: " + roleList);
...
//            }
//            
//                                        String companyId = new UserProfileService().getRoleParameters("[2500000000000067]",
//                        userSession.getUser().getUser_code());
            ...
            if (allowUpdate) {
                ...
                if (apiData.getService().equalsIgnoreCase("channels")) {
                    ...
                    if (roleList.contains("2")) {
                        ...
                        System.out.println("bank transfer insert started" + request);
//                        String reference = request.getParameter("transid");
//                        System.out.println("Reference" + reference);
//                        String transid = request.getParameter("transid");
//                        System.out.println("transid" + transid);

                        String records = new FundsTransferDao().callInsertData(new Gson().toJson(apiData));
                ...
                } else if (apiData.getService().equalsIgnoreCase("fundstransfertransactions")) {
                    System.out.println("Fundstransfer transactions started");
                    if (allowUpdate) {
                        List<FundsTransfer> records = new FundsTransferDao().getFundsTransferTransaction(new Gson().toJson(apiData));
                ...
                }else if (apiData.getService().equalsIgnoreCase("fundstransfertransactionsall")) {
                    System.out.println("Fundstransfer all transactions started");
                    if (allowUpdate) {
                        List<FundsTransfer> records = new FundsTransferDao().getFundsTransferTransactionAll(new Gson().toJson(apiData));
                ...
                else if (apiData.getService().equalsIgnoreCase("authorizeTransfer")) {
                    if (roleList.contains("3") || roleList.contains("1")) {

                        System.out.println("Funds transfer authorize now started" + request);

                        String records = new FundsTransferDao().callAuthorizeTransfer(new Gson().toJson(apiData));
                ...
                }else if (apiData.getService().equalsIgnoreCase("sourceaccounts")) {
                    
                    System.out.println("fetching list of account ");

                             List<Account> records = new FundsTransferDao().getSourceCompanyAccounts(companyId);
                              out.println(new Gson().toJson(records));

                }
                
                else if (apiData.getService().equalsIgnoreCase("accounts")) {
                    
                    System.out.println("fetching list of account ");

                             List<Account> records = new FundsTransferDao().getCompanyAccounts(companyId);
                              out.println(new Gson().toJson(records));

                }else if (apiData.getService().equalsIgnoreCase("cardbalance")) {
                    
                    System.out.println("fetching list of source cards with amounts on them");

                             List<Account> records = new FundsTransferDao().getCardBalance(companyId);
                              out.println(new Gson().toJson(records));
